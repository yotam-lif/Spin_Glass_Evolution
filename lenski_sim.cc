...
void lenski_sim::dilute_gsl() {
    ...
    vector<vector<int>> new_mut_times; // @Yotam
    ...
    while (nsamps > 0) {
        ...
        if (strain == n_strains-1) {
            ...
            n_choices = gsl_ran_binomial_tpe(gsl_gen, nbac_curr/(nbac_curr + nbac_other), nsamps);
        ...
        if (n_choices > 0) {
            ...
            if (hoc) {
                new_mutations_hoc.push_back(mutations_hoc[strain_ind]);
            } else {
                new_mutations.push_back(mutations[strain_ind]);
            }
            new_mut_order.push_back(mut_order[strain_ind]);
            new_mut_times.push_back(mut_times[strain_ind]); // @Yotam
    ...
    mut_times.swap(new_mut_times); // @Yotam
...
void lenski_sim::output_dom_dfe_data() {
    ...
    for (auto strain_mut_order : dom_mut_order) {
        if (!strain_mut_order.empty()) {
            for (int i = 0; i < strain_mut_order.size() - 1; i++) {
                dom = strain_mut_order[i];
                dom_outFile << dom << " ";
            }
            dom_outFile << strain_mut_order[strain_mut_order.size() - 1];
        }
